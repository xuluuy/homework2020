...
export default {
    ...
    data () {
      ...
      if(!value){callback(new Error('请拖动滑块完成验证'))}
       else if(!this.msg){callback(new Error("请重新拖动滑块完成验证"))}
       else {
        callback();
      }
    };
        ...
        cur:0 ,//默认选中第一个tab
         rules: {
          ...
          password: [
           ...
           check:[
            { required: true, trigger: 'change',validator: validCheck },
         ]  
        }
      }
    },
